...
TAX_CODE_MAP = {
    ...
    '9996': ('11', '9996', 'GRA',  'FRE', 'Z'),   # Gratuita (retiro/transferencia gratuita)
}
...
UOM_CODE_BY_XMLID = {
    'uom.product_uom_unit': 'NIU', 'uom.product_uom_dozen': 'DPC',
    'uom.product_uom_kgm': 'KGM', 'uom.product_uom_gram': 'GRM',
    'uom.product_uom_day': 'DAY', 'uom.product_uom_hour': 'HUR',
    'uom.product_uom_ton': 'TNE', 'uom.product_uom_meter': 'MTR',
    'uom.product_uom_km': 'KTM', 'uom.product_uom_cm': 'CMT',
    'uom.product_uom_litre': 'LTR', 'uom.product_uom_lb': 'LBR',
    'uom.product_uom_oz': 'ONZ', 'uom.product_uom_inch': 'INH',
    'uom.product_uom_foot': 'FOT', 'uom.product_uom_mile': 'M52',
    'uom.product_uom_floz': 'OZ', 'uom.product_uom_qt': 'QTI',
...
class AccountMove(models.Model):
    ...
    l10n_pe_biller_state = fields.Selection(
        selection=[('por_enviar', 'Por enviar'), ('enviado', 'Enviado'),
                   ('anulado', 'Anulado'), ('rechazado', 'Rechazado'), ('error', 'Error')],
        string='Estado Facturador', default='por_enviar', copy=False)
    l10n_pe_ne_baja_motivo = fields.Char(string='Motivo de baja', copy=False,
        help="Motivo de la comunicación de baja (RA) del comprobante ante SUNAT.")
    l10n_pe_ne_baja_correlativo = fields.Char(string='Correlativo RA', copy=False, readonly=True)
    l10n_pe_ne_baja_fecha = fields.Date(string='Fecha RA', copy=False, readonly=True)
    l10n_pe_ne_baja_doc = fields.Char(string='Comunicación de baja', compute='_compute_l10n_pe_ne_baja_doc',
        store=True, help="Identificador de la comunicación de baja: RA-AAAAMMDD-correlativo.")
    l10n_pe_ne_baja_cdr = fields.Many2one('ir.attachment', string='CDR de baja', copy=False)
    ...
    l10n_pe_ne_tipo_doc = fields.Char(string='Tipo doc. emitido', copy=False, readonly=True)
    l10n_pe_ne_serie_emit = fields.Char(string='Serie emitida', copy=False, readonly=True)
    l10n_pe_ne_corr_emit = fields.Char(string='Correlativo emitido', copy=False, readonly=True)
    l10n_pe_serie = fields.Char(
        string='Serie', compute='_compute_l10n_pe_serie', store=True, readonly=False, copy=False,
        help="Serie del comprobante. Por defecto, la del diario (l10n_pe_ne_serie).")
    l10n_pe_correlativo = fields.Char(
        string='Correlativo', copy=False,
    ...
    l10n_pe_ne_detraccion_code = fields.Char(
        string='Código detracción', copy=False,
        help="Código del bien/servicio sujeto a detracción (catálogo 54 de SUNAT, ej. 037).")
    l10n_pe_ne_detraccion_rate = fields.Float(string='% Detracción', digits=(5, 2), copy=False)
    l10n_pe_ne_detraccion_medio_pago = fields.Char(
        string='Medio de pago detracción', default='001', copy=False,
    ...
    l10n_pe_ne_percepcion_rate = fields.Float(string='% Percepción', digits=(5, 2), default=2.0, copy=False)
    l10n_pe_ne_anticipo_total = fields.Monetary(
        string='Anticipo aplicado (total con IGV)', copy=False,
    ...
    l10n_pe_ne_anticipo_doc = fields.Char(
        string='Comprobante de anticipo', copy=False, help="Serie-correlativo de la factura de anticipo (ej. F001-00000100).")
    l10n_pe_ne_anticipo_tipo = fields.Selection(
        [('02', 'Factura de anticipo'), ('03', 'Boleta de anticipo')],
        string='Tipo doc. anticipo', default='02', copy=False,
    ...
    def _l10n_pe_importe_cobrar(self):
        ...
        return round(self.amount_total - (ant[2] if ant else 0.0) - self._l10n_pe_gratuito_base(), 2)
    ...
    def _l10n_pe_percepcion_monto(self):
        ...
        return round(self._l10n_pe_importe_cobrar() * (self.l10n_pe_ne_percepcion_rate or 0.0) / 100.0, 2)
    ...
    def _l10n_pe_anticipo_gravado(self):
        ...
        tasa = next((self._l10n_pe_tax_info(l)[1] for l in lines
                     if self._l10n_pe_tax_info(l)[0][1] == cod_tri), 18.0)
    ...
    def _l10n_pe_check_anticipo(self):
        ...
        if not (self.l10n_pe_ne_anticipo_doc or '').strip():
            raise UserError(_("Indique el comprobante de anticipo (serie-correlativo, ej. F001-00000100)."))
        if round(self.l10n_pe_ne_anticipo_total, 2) > self.amount_total + 0.01:
            raise UserError(_("El anticipo aplicado (%.2f) no puede exceder el total de la factura (%.2f).")
    ...
    def _l10n_pe_variables_globales(self):
        ...
        if self.l10n_pe_ne_percepcion:
            out.append({
                ...
                "porVariableGlobal": "%.2f" % (self.l10n_pe_ne_percepcion_rate / 100.0),
    ...
    def _l10n_pe_detraccion_monto(self):
        self.ensure_one()
        return round(self.amount_total * (self.l10n_pe_ne_detraccion_rate or 0.0) / 100.0, 2)
    ...
    def _l10n_pe_adicional_cabecera(self):
        ...
        if self.l10n_pe_ne_detraccion:
            block.update({
                "ctaBancoNacionDetraccion": self.company_id.l10n_pe_ne_cuenta_detraccion or "",
        ...
        if self.l10n_pe_ne_percepcion:
            ...
            block["mtoTotPercepcion"] = fmt(self._l10n_pe_importe_cobrar() + self._l10n_pe_percepcion_monto())
    ...
    l10n_pe_biller_xml = fields.Many2one('ir.attachment', string='XML UBL firmado', copy=False)
    l10n_pe_biller_cdr = fields.Many2one('ir.attachment', string='CDR SUNAT', copy=False)
    l10n_pe_biller_pdf = fields.Many2one('ir.attachment', string='PDF (representación impresa)', copy=False)
    ...
    def _l10n_pe_document_type(self):
        ...
        if self.move_type == 'out_invoice' and self._l10n_pe_tipo_operacion() == '0200':
            return '01'
        vat_code = self.partner_id.l10n_latam_identification_type_id.l10n_pe_vat_code or ''
        return '01' if vat_code == '6' else '03'

    def _l10n_pe_product_lines(self):
        return self.invoice_line_ids.filtered(lambda l: not l.display_type or l.display_type == 'product')
    ...
    def _l10n_pe_line_amounts(self, line):
        ...
        icbper = round(int(round(line.quantity or 0)) * icbper_tax.amount, 2) if icbper_tax else 0.0
    ...
    def _l10n_pe_total_icbper(self):
        return sum(self._l10n_pe_line_amounts(l)[3] for l in self._l10n_pe_product_lines())
    ...
    def _l10n_pe_detalle(self):
        ...
        for line in self._l10n_pe_product_lines():
            (tip_afe, cod_tri, nom_trib, cod_tip_trib, _cod_cat), por_igv = self._l10n_pe_tax_info(line)
            ...
            disc = round(line.price_unit * line.quantity - base, 2) if line.discount else 0.0
            gross = base + disc
            item = {
                "tipAfeIGV": tip_afe,
            ...
            isc_tax = self._l10n_pe_isc_tax(line)
            if isc_tax:
                por_isc = isc_tax.amount if isc_tax.amount_type != 'fixed' else (isc / base * 100.0 if base else 0.0)
                item.update({
                    "codTriISC": "2000",
    ...
    def _l10n_pe_tributos(self):
        ...
        for line in self._l10n_pe_product_lines():
            (_tip, cod_tri, nom_trib, cod_tip_trib, cod_cat), _por = self._l10n_pe_tax_info(line)
            ...
            g = grupos.setdefault(cod_tri, [0.0, 0.0, (nom_trib, cod_tip_trib, cod_cat)])
            g[0] += base
        ...
        tributos = [{
            "ideTributo": cod_tri, "nomTributo": meta[0], "codTipTributo": meta[1],
            "codCatTributo": meta[2], "mtoBaseImponible": fmt(b), "mtoTributo": fmt(m),
    ...
    def _l10n_pe_leyendas(self):
        ...
        leyendas = [{"codLeyenda": "1000", "desLeyenda": leyenda_monto(self._l10n_pe_importe_cobrar())}]
        if self.l10n_pe_ne_detraccion:
            leyendas.append({"codLeyenda": "2006", "desLeyenda": "Operacion sujeta a detraccion"})
        if self._l10n_pe_gratuito_base() > 0:
            leyendas.append({"codLeyenda": "1002", "desLeyenda": "TRANSFERENCIA GRATUITA"})
        return leyendas
    ...
    def _l10n_pe_cabecera(self):
        ...
        cabecera = {
            ...
            "fecEmision": self.invoice_date.strftime("%Y-%m-%d") if self.invoice_date else "",
            "horEmision": fields.Datetime.now().strftime("%H:%M:%S"),
            "fecVencimiento": self.invoice_date_due.strftime("%Y-%m-%d") if self.invoice_date_due else "",
            "codLocalEmisor": "0000",
    ...
    def _l10n_pe_build_invoice_request(self):
        ...
        if self._l10n_pe_document_type() == '03' and (self.amount_total or 0.0) > 700 \
                and not (self.partner_id.vat or '').strip():
            raise UserError(_("Una boleta mayor a S/ 700 requiere el documento de identidad del cliente."))
        req = {
    ...
    def _l10n_pe_build_note_request(self):
        ...
        cabecera["codMotivo"] = self.l10n_pe_motivo_code or ('01' if dt == '07' else '02')
        if dt == '08':
            cabecera["desMotivo"] = ND_MOTIVO_DESC.get(cabecera["codMotivo"], "Aumento en el valor")
        req = {
        ...
        if dt == '07':
            ...
            moneda = self.currency_id.name or "PEN"
            req["datoPago"] = {"formaPago": "Credito", "mtoNetoPendientePago": total,
                               "tipMonedaMtoNetoPendientePago": moneda}
            req["detallePago"] = [{"mtoCuotaPago": total, "fecCuotaPago": fecha,
                                   "tipMonedaCuotaPago": moneda}]
        return req
    ...
    def _l10n_pe_store_cdr(self, cdr_b64):
        ...
        try:
            ...
            return '', ''
        serie, correlativo = self._l10n_pe_serie_correlativo()
        name = 'R%s-%s-%s.zip' % (self.company_id.vat or '', serie, correlativo.zfill(8))
        att = self.env['ir.attachment'].create({
            'name': name, 'res_model': 'account.move', 'res_id': self.id,
    ...
    def action_l10n_pe_send_to_biller(self):
        ...
        for move in self:
            ...
            try:
                resp = requests.post(base + '/generator/' + endpoint, json=payload, headers=headers, timeout=timeout)
            except requests.RequestException as exc:
                move.l10n_pe_biller_state = 'error'
                move.l10n_pe_biller_message = _("Error de conexión con el facturador: %s") % exc
                continue
            signed = any(tag in resp.text for tag in ('<Invoice', '<CreditNote', '<DebitNote'))
            if resp.status_code == 200 and signed:
                serie, correlativo = move._l10n_pe_serie_correlativo()
                att = self.env['ir.attachment'].create({
                    'name': '%s-%s-%s.xml' % (move.company_id.vat, serie, correlativo.zfill(8)),
                ...
                if code == '0':
                    move.l10n_pe_biller_message = _("Aceptado por SUNAT — CDR ResponseCode 0. %s") % (desc or '')
                elif code:
                    move.l10n_pe_biller_message = _("CDR de SUNAT (ResponseCode %s). %s") % (code, desc or '')
                else:
                    move.l10n_pe_biller_message = _("Aceptado por el facturador (HTTP 200).")
            else:
    ...
    def _l10n_pe_get_pdf_attachment(self):
        ...
        if not self.l10n_pe_biller_xml:
            raise UserError(_("El comprobante no tiene XML firmado; envíelo primero a SUNAT."))
        icp = self.env['ir.config_parameter'].sudo()
        base = icp.get_param('l10n_pe_ne_biller.url', 'http://localhost:8090').rstrip('/')
        timeout = int(icp.get_param('l10n_pe_ne_biller.timeout', '60'))
        ...
        att = self.env['ir.attachment'].create({
            'name': '%s-%s-%s.pdf' % (self.company_id.vat or '', serie, correlativo.zfill(8)),
        ...
        with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
            for move in self:
                if move.l10n_pe_biller_xml:
                    zf.writestr(move.l10n_pe_biller_xml.name, move.l10n_pe_biller_xml.raw or b'')
                    incluidos += 1
                if move.l10n_pe_biller_cdr:
                    zf.writestr(move.l10n_pe_biller_cdr.name, move.l10n_pe_biller_cdr.raw or b'')
                    incluidos += 1
                try:
                    pdf = move._l10n_pe_get_pdf_attachment() if move.l10n_pe_biller_xml else False
                    if pdf:
                        zf.writestr(pdf.name, pdf.raw or b'')
                        incluidos += 1
                except UserError as exc:
                    # PDF best-effort: si el micro falla, el ZIP igual lleva XML + CDR; se registra el motivo.
                    _logger.warning("No se pudo generar el PDF de %s para el ZIP: %s", move.name, exc)
        if not incluidos:
            raise UserError(_("Los comprobantes seleccionados no tienen archivos para descargar."))
        att = self.env['ir.attachment'].create({
            'name': 'comprobantes_sunat.zip', 'mimetype': 'application/zip',
            'raw': buf.getvalue()})
        return self._l10n_pe_download_url(att)

    # ---------------------------------------------- comunicación de baja (RA)
    @api.depends('l10n_pe_ne_baja_fecha', 'l10n_pe_ne_baja_correlativo', 'l10n_pe_ne_tipo_doc')
        ...
        for m in self:
            if m.l10n_pe_ne_baja_fecha and m.l10n_pe_ne_baja_correlativo:
                ...
                m.l10n_pe_ne_baja_doc = '%s-%s-%s' % (
                    prefijo, m.l10n_pe_ne_baja_fecha.strftime('%Y%m%d'), m.l10n_pe_ne_baja_correlativo)
            else:
    ...
    def _l10n_pe_check_baja(self):
        ...
        if self.l10n_pe_biller_state not in ('enviado', 'anulado'):
            raise UserError(_("Solo puede comunicarse la baja de un comprobante ya enviado a SUNAT."))
        ...
        if not re.match(r'^([BFS][A-Z0-9]{3}|\d{1,4})$', serie or ''):
            raise UserError(_("La serie del comprobante (%s) no tiene un formato válido para la anulación.") % serie)
        ...
        if tipo == '01':
            limite = int(self.env['ir.config_parameter'].sudo().get_param('l10n_pe_ne_biller.baja_plazo_dias', '7'))
            ...
            if dias > limite:
                raise UserError(_(
                    ...
                    "(máximo %s días calendario). Para anularla, emita una nota de crédito.") % (dias, limite))
        # Boleta mayor a S/ 700 exige el documento de identidad del adquirente (igual que en su emisión).
        if tipo == '03' and (self.amount_total or 0.0) > 700 and not (self.partner_id.vat or '').strip():
            raise UserError(_(
    ...
    _RC_CATEGORIA = {'1000': 'gravado', '1016': 'gravado', '9997': 'exonerado',
                     '9998': 'inafecto', '9995': 'exportado', '9996': 'gratuito'}

    def _l10n_pe_rc_totales(self):
        """Totales de valor por categoría (gravado/exonerado/inafecto/exportado/gratuito) para el RC."""
        self.ensure_one()
        cats = dict.fromkeys(('gravado', 'exonerado', 'inafecto', 'exportado', 'gratuito'), 0.0)
        for line in self._l10n_pe_product_lines():
    ...
    def _l10n_pe_build_rc_request(self):
        ...
        tributos = [{
            "idLineaRd": "1",
            "ideTributoRd": t["ideTributo"], "nomTributoRd": t["nomTributo"],
            "codTipTributoRd": t["codTipTributo"],
            "mtoBaseImponibleRd": t["mtoBaseImponible"], "mtoTributoRd": t["mtoTributo"],
        ...
        if icbper:
            tributos.append({"idLineaRd": "1", "ideTributoRd": "7152", "nomTributoRd": "ICBPER",
        ...
        if not any(t["ideTributoRd"] == "1000" for t in tributos):
            tributos.append({"idLineaRd": "1", "ideTributoRd": "1000", "nomTributoRd": "IGV",
    ...
    def _l10n_pe_store_baja_cdr(self, cdr_b64):
        ...
        try:
            ...
            return '', ''
        att = self.env['ir.attachment'].create({
            'name': 'R%s-%s.zip' % (self.company_id.vat or '', self.l10n_pe_ne_baja_doc or 'RA'),
            'res_model': 'account.move', 'res_id': self.id,
            'mimetype': 'application/zip', 'raw': cdr_bytes})
    ...
    def action_l10n_pe_send_baja(self):
        ...
        for move in self:
            ...
            seq, endpoint, root = (('l10n_pe.ne.rc', '/generator/resumenBoleta', '<SummaryDocuments')
                                   if es_boleta else ('l10n_pe.ne.ra', '/generator/resumenBaja', '<VoidedDocuments'))
            # Correlativo del resumen asignado una sola vez; un reintento limpio reusa el mismo.
            if not move.l10n_pe_ne_baja_correlativo:
                move.l10n_pe_ne_baja_correlativo = self.env['ir.sequence'].next_by_code(seq) or '1'
                move.l10n_pe_ne_baja_fecha = fields.Date.context_today(move)
            ra = move.l10n_pe_ne_baja_doc
            payload = move._l10n_pe_build_rc_request() if es_boleta else move._l10n_pe_build_baja_request()
            headers = {'X-Api-Key': move.company_id.sudo().l10n_pe_ne_api_key or ''}
            try:
                resp = requests.post(base + endpoint, json=payload, headers=headers, timeout=timeout)
            except requests.RequestException as exc:
                # Nunca llegó a SUNAT: libera el resumen para reintentar con uno fresco.
                move._l10n_pe_baja_liberar()
                move.l10n_pe_biller_message = _("Error de conexión con el facturador (anulación %s): %s") % (ra, exc)
                continue
            if resp.status_code == 200 and root in resp.text:
                cdr_b64 = resp.headers.get('X-Sunat-Cdr')
                code, desc = move._l10n_pe_store_baja_cdr(cdr_b64) if cdr_b64 else ('', '')
                if code == '0':
                    move.l10n_pe_biller_state = 'anulado'
                    move.l10n_pe_biller_message = _(
                        "Anulación %s aceptada por SUNAT (ResponseCode 0). %s") % (ra, desc or '')
                elif code:
                    # SUNAT lo recibió y rechazó: libera el resumen; el usuario corrige y reintenta con uno nuevo.
                    move._l10n_pe_baja_liberar()
                    move.l10n_pe_biller_message = _(
                        "Anulación %s rechazada por SUNAT (ResponseCode %s). %s") % (ra, code, desc or '')
                else:
                    ...
                    move.l10n_pe_biller_message = _(
                        "Anulación %s enviada; respuesta de SUNAT indeterminada (sin CDR legible). Verifique en SUNAT.") % ra
            else:
                move._l10n_pe_baja_liberar()
                move.l10n_pe_biller_message = _("Anulación %s rechazada por el facturador: %s") % (ra, (resp.text or '')[:1200])
        return True
